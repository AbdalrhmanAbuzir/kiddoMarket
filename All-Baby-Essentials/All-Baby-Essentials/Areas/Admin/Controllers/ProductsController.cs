...
namespace All_Baby_Essentials.Areas.Admin.Controllers
{
    ...
    public class ProductsController : Controller
    {
        ...
        {
            var products = await _context.Products.Include(p => p.Category).Include(p => p.Images).Where(p => !p.IsDeleted).ToListAsync();
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            var product = await _context.Products.Include(p => p.Category).Include(p => p.Images).Include(p => p.Reviews).ThenInclude(u => u.User).FirstOrDefaultAsync(p => p.Id == id && !p.IsDeleted);
